...
pipeline {
    ...
    environment {
        ...
        stable_revision = sh(script: 'curl -H "Authorization: Basic ZGF5YWthci5nQGhjbC5jb206TmV2ZXI0Z3QwMSE=" "https://api.enterprise.apigee.com/v1/organizations/dayakarg-eval/apis/HelloWorld/deployments" | jq -r ".environment[0].revision[0].name"', returnStdout: true).trim()
    }
